...
// ...existing code...
<Layout>
  <div className="w-full min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 grid grid-cols-1 sm:grid-cols-[1.1fr_4fr]">
    <Side_bar />
    <div className="flex p-8 min-h-screen w-full relative flex-col gap-10 px-4 sm:px-20">
      <div className="p-4">
        <h1 className="text-3xl font-extrabold text-green-700 tracking-tight mb-2">
      ...
      <div className="rounded-2xl shadow-2xl border bg-white border-gray-200 p-8 max-w-3xl mx-auto">
        <div className="flex flex-col md:flex-row items-center justify-between mb-8 gap-6">
          <div className="flex items-center gap-6">
            <div className="bg-gradient-to-br from-green-100 to-emerald-100 p-2 rounded-xl shadow">
              <Image
                height={100}
                width={100}
                src={"/doliprane.jpg"}
                alt={foundMedicine.brandName}
                className="w-24 h-24 object-contain rounded-lg border border-green-200"
              />
            </div>
            <div>
              <h2 className="text-2xl font-bold text-gray-800 mb-1">{foundMedicine.brandName}</h2>
              <Link href={`./${id}/${foundMedicine.genericName}`}>
                <span className="text-green-600 bg-green-50 px-3 py-1 rounded-full text-sm font-medium hover:bg-green-100 transition">
          ...
          <Link href={`./${id}/${foundMedicine.genericName}`}>
            <button
              className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 text-white font-semibold py-2 px-6 rounded-lg shadow-lg transition-all duration-200 flex items-center gap-2"
            >
